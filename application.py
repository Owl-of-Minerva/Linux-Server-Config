...
import sys
sys.path.append("/var/www/html/Catalog_Application")
from modules import database_setup
...
app = Flask(__name__)
app.secret_key = 'qPEeBw9QLWYrYSR7rvEP0GEs'
CLIENT_ID = json.loads(
    open('/var/www/html/Catalog_Application/client_secrets.json', 'r').read())['web']['client_id']
...
# add new user to the User dataset
...
# get user information using the user_id
...
# the page displaying all categories and latest items
@app.route('/')
@app.route('/catalog/')
def showCatalog():
...
def gconnect():
    ...
    try:
        ...
        oauth_flow = flow_from_clientsecrets('/var/www/html/Catalog_Application/client_secrets.json', scope='')
...
def fbconnect():
    ...
    app_id = json.loads(open('/var/www/html/Catalog_Application/fb_client_secrets.json', 'r').read())[
        'web']['app_id']
    app_secret = json.loads(
        open('/var/www/html/Catalog_Application/fb_client_secrets.json', 'r').read())['web']['app_secret']
...
def catalogJson():
    ...
    for category in categories:
        ...
        category_json['Item'] = items_json
        categories_json.append(category_json)
    return jsonify(Category=[c for c in categories_json])
...
# show items belonging to one particular category
...
def showItem(category_name, item_name):
    if 'email' in login_session:
        ...
        user = session.query(User).filter_by(email=email).one()
        item = session.query(Item).filter_by(name=item_name).one()
        return render_template('item.html', item=item, user_id=user.id)
    else:
        item = session.query(Item).filter_by(name=item_name).one()
        return render_template('item.html', item=item, user_id=-1)
